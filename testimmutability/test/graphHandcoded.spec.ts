...
import {GNode, Graph, SelectionKind} from '../src/models/GraphHandcoded'
import {expect} from 'chai'

describe('GraphHandcoded: access and code completion', () => {
  // constructor
  ...
  it("is not type compatible with plain javascript object", ()=> {
    ...
    let g:GNode = {title: "test",id:1,x:1,y:2,width:10,height:10,selection:SelectionKind.None, getRatio:()=>1, setTitle:(v:string)=>this,setId:(v:number)=>this,setX:(v:number)=>this, setY:(v:number)=>this, setWidth:(v:number)=>this,setHeight:(v:number)=>this, setSelection:(v:SelectionKind) => this};
    expect(g.title).equals("test");
    expect(g.getRatio()).equals(1);
  });
});

describe('GraphHandcoded: graph.moveNodes', () => {
  var graph;
  var initialNodes;
  beforeEach(() => {
